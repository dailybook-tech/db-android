...
import co.dailybook.base.analytics.FacebookPaymentEvents
import co.dailybook.base.datastore.DataStoreManager
import co.dailybook.keep.R
import co.dailybook.keep.databinding.DialogPremiumOfferBinding
...
class PremiumOfferDialogFragment : DialogFragment() {
    ...
    private var selectedPlan: co.dailybook.keep.model.subscription.SubscriptionPlan? = null
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        dialog.window?.apply {
            ...
            statusBarColor = resources.getColor(co.dailybook.keep.R.color.background, null)
    ...
    private fun setupViews() {
        binding?.apply {
            ...
            // razorpayCustom = Razorpay(requireActivity(), co.dailybook.keep.BuildConfig.RAZORPAY_KEY_ID)
            // fetchPaymentMethods()
            ...
            videoSubscriptionExplainer.setVideoURI(
                Uri.parse("android.resource://${requireContext().packageName}/${co.dailybook.base.R.raw.subscription_explainer_video}")
            )
    ...
    private fun handlePlansLoaded(plans: List<co.dailybook.keep.model.subscription.SubscriptionPlan>) {
        if (plans.isNotEmpty()) {
            // Filter active plans
    ...
    private fun updateTrialHeadline(plan: co.dailybook.keep.model.subscription.SubscriptionPlan) {
        binding?.apply {
            if (isTrialExpired) {
    ...
    private fun updatePriceDisplay(plan: co.dailybook.keep.model.subscription.SubscriptionPlan) {
        binding?.apply {
    ...
    private fun bindPlanItem(
        ...
        plan: co.dailybook.keep.model.subscription.SubscriptionPlan,
        isSelected: Boolean
    ) {
        itemBinding.apply {
    ...
    private fun getStartTrialButtonText(plan: co.dailybook.keep.model.subscription.SubscriptionPlan?): CharSequence {
    ...
    private fun populateFeaturesList(plan: co.dailybook.keep.model.subscription.SubscriptionPlan) {
    ...
    private fun initiateRazorpayPayment(response: co.dailybook.keep.model.subscription.CreateSubscriptionResponse) {
    ...
    private fun initiatePaymentWithSubscription(
        response: co.dailybook.keep.model.subscription.CreateSubscriptionResponse,
