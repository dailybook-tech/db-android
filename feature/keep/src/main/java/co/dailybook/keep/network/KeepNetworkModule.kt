...
class KeepNetworkModule(val db: AppDatabase) {
    ...
    suspend fun addOrUpdateSalary(
        userId: String,
        body: co.dailybook.keep.model.AddOrUpdateSalaryRequestBody
    ): Flow<NetworkResult<String?>> {
        return networkHandler.getData {
    ...
    suspend fun getSubscriptionPlans(userId: String): Flow<NetworkResult<co.dailybook.keep.model.subscription.SubscriptionPlansResponse?>> {
        return networkHandler.getData { api.getSubscriptionPlans(userId) }
    }
    
    suspend fun getUserSubscription(userId: String): Flow<NetworkResult<co.dailybook.keep.model.subscription.UserSubscription?>> {
        return networkHandler.getData { api.getUserSubscription(userId) }
    }
    
    suspend fun createSubscription(
        userId: String,
        request: co.dailybook.keep.model.subscription.CreateSubscriptionRequest
    ): Flow<NetworkResult<co.dailybook.keep.model.subscription.CreateSubscriptionResponse?>> {
        return networkHandler.getData { api.createSubscription(userId, request) }
    }
    
    suspend fun verifySubscription(
        subscriptionId: String,
        request: co.dailybook.keep.model.subscription.VerifySubscriptionRequest
    ): Flow<NetworkResult<co.dailybook.keep.model.subscription.VerifySubscriptionResponse?>> {
        return networkHandler.getData { api.verifySubscription(subscriptionId, request) }
    }
    
    suspend fun cancelSubscription(
        subscriptionId: String
    ): Flow<NetworkResult<co.dailybook.keep.model.subscription.CancelSubscriptionResponse?>> {
