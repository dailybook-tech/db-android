...
class KeepNetworkModule(val db: AppDatabase) {
    ...
    suspend fun addOrUpdateSalary(
        userId: String,
        body: com.laborbook.keep.model.AddOrUpdateSalaryRequestBody
    ): Flow<NetworkResult<String?>> {
        return networkHandler.getData {
    ...
    suspend fun getSubscriptionPlans(userId: String): Flow<NetworkResult<com.laborbook.keep.model.subscription.SubscriptionPlansResponse?>> {
        return networkHandler.getData { api.getSubscriptionPlans(userId) }
    }
    
    suspend fun getUserSubscription(userId: String): Flow<NetworkResult<com.laborbook.keep.model.subscription.UserSubscription?>> {
        return networkHandler.getData { api.getUserSubscription(userId) }
    }
    
    suspend fun createSubscription(
        userId: String,
        request: com.laborbook.keep.model.subscription.CreateSubscriptionRequest
    ): Flow<NetworkResult<com.laborbook.keep.model.subscription.CreateSubscriptionResponse?>> {
        return networkHandler.getData { api.createSubscription(userId, request) }
    }
    
    suspend fun verifySubscription(
        subscriptionId: String,
        request: com.laborbook.keep.model.subscription.VerifySubscriptionRequest
    ): Flow<NetworkResult<com.laborbook.keep.model.subscription.VerifySubscriptionResponse?>> {
        return networkHandler.getData { api.verifySubscription(subscriptionId, request) }
    }
    
    suspend fun cancelSubscription(
        subscriptionId: String
    ): Flow<NetworkResult<com.laborbook.keep.model.subscription.CancelSubscriptionResponse?>> {
