...
class PremiumOfferDialogFragment : DialogFragment(), PaymentResultWithDataListener {
    ...
    private var selectedPlan: com.laborbook.keep.model.subscription.SubscriptionPlan? = null
    ...
    /** Razorpay Custom UI SDK instance (used for subscription_id–based UPI payment). */
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        dialog.window?.apply {
            ...
            statusBarColor = resources.getColor(com.laborbook.keep.R.color.background, null)
    ...
    private fun setupViews() {
        binding?.apply {
            ...
            razorpayCustom = Razorpay(requireActivity(), com.laborbook.keep.BuildConfig.RAZORPAY_KEY_ID)
            // Intentionally not calling setWebView() here so our client stays and can open UPI apps
            fetchPaymentMethods() // Razorpay Custom SDK 1.4: fetch enabled payment methods
            ...
            videoSubscriptionExplainer.setVideoURI(
                Uri.parse("android.resource://${requireContext().packageName}/${com.laborbook.base.R.raw.subscription_explainer_video}")
            )
    ...
    private fun handlePlansLoaded(plans: List<com.laborbook.keep.model.subscription.SubscriptionPlan>) {
        if (plans.isNotEmpty()) {
            // Filter active plans
    ...
    private fun updateTrialHeadline(plan: com.laborbook.keep.model.subscription.SubscriptionPlan) {
        binding?.apply {
            if (isTrialExpired) {
    ...
    private fun updatePriceDisplay(plan: com.laborbook.keep.model.subscription.SubscriptionPlan) {
        binding?.apply {
    ...
    private fun bindPlanItem(
        ...
        plan: com.laborbook.keep.model.subscription.SubscriptionPlan,
        isSelected: Boolean
    ) {
        itemBinding.apply {
    ...
    private fun getStartTrialButtonText(plan: com.laborbook.keep.model.subscription.SubscriptionPlan?): CharSequence {
    ...
    private fun populateFeaturesList(plan: com.laborbook.keep.model.subscription.SubscriptionPlan) {
    ...
    private fun fetchPaymentMethods() {
        razorpayCustom?.getPaymentMethods(object : PaymentMethodsCallback {
            override fun onPaymentMethodsReceived(result: String?) {
                // JSON structure: https://api.razorpay.com/v1/methods?key_id=...
    ...
    // UPI AutoPay plan alignment:
    // 2. SDK: com.razorpay:customui:3.9.22, Razorpay(activity, key). Checkout.preload is for Standard SDK; we use Custom UI only.
    // 3. WebView: setWebView(webView) before submit with subscription_id path.
    // 4. Trigger: upi_intent_uri → direct open; else submit with subscription_id + recurring=1 (mandate).
    // 5–6. UPI intent + onActivityResult: we pass _[flow]=intent, upi_app_package_name; Activity forwards UPI_INTENT_REQUEST_CODE to razorpay.onActivityResult.
    // 7. Verify: we send payment_id, subscription_id, signature to backend via verifySubscription(); server verifies HMAC (plan step 7).

    /** Initiates payment via Razorpay SDK using subscription_id and UPI intent params (no order_id / customer_id). */
    private fun initiateRazorpayPayment(response: com.laborbook.keep.model.subscription.CreateSubscriptionResponse) {
    ...
    /** Uses Razorpay SDK with subscription_id + UPI intent params. */
    private fun initiatePaymentWithSubscription(
        response: com.laborbook.keep.model.subscription.CreateSubscriptionResponse,
        ...
        // For trial: UPI mandate amount is INR 5 (500 paise), refunded so UI shows 0. After trial, full amount is charged. No trial: charge full recurring amount now.
        val amountPaise = if (plan.hasTrial) 500 else (plan.discountedPrice * 100)
        Timber.d("Razorpay checkout amount: amountPaise=$amountPaise hasTrial=${plan.hasTrial} discountedPrice=${plan.discountedPrice} planId=${plan.id} currency=${plan.currency}")
        if (amountPaise < 100) {
        ...
        lifecycleScope.launch {
            val mobileNumber = dataStoreManager.read(DataStoreManager.MOBILE_NUMBER, "").first()
            val userId = dataStoreManager.read(DataStoreManager.USER_ID, "").first()
            val email = "${userId.take(8)}@laborbook.app"
            val upiAppPackageName = upiApp.packageName

            // UPI Autopay Intent payload per Razorpay doc: required fields only
            ...
            // Re-apply our WebViewClient so UPI/intent URLs open in external app; setWebView() can overwrite it
            webView.webViewClient = createPaymentWebViewClient()
            webView.visibility = View.VISIBLE
            Timber.d("Razorpay submit payload: amount=$amountPaise currency=${plan.currency} subscription_id=$subscriptionId")
            try {
    ...
    override fun onPaymentSuccess(razorpayPaymentId: String, paymentData: PaymentData?) {
        Timber.d("onPaymentSuccess: razorpayPaymentId=$razorpayPaymentId currentSubscriptionId=$currentSubscriptionId paymentData=${paymentData?.data?.toString()?.take(200)}")
        hidePaymentWebView()
        ...
        // Extract subscription data from payment
        val razorpaySubscriptionId = paymentData?.data?.optString("razorpay_subscription_id") ?: ""
        // Signature can be in either paymentData.signature OR paymentData.data.razorpay_signature
        val signature = paymentData?.data?.optString("razorpay_signature") 
            ?: paymentData?.signature 
            ?: ""
        
        if (currentSubscriptionId != null && razorpaySubscriptionId.isNotEmpty()) {
            Timber.d("onPaymentSuccess: starting verification polling subscriptionId=$currentSubscriptionId razorpaySubscriptionId=$razorpaySubscriptionId hasSignature=${signature.isNotEmpty()}")
            ...
            Timber.e("onPaymentSuccess: missing data - currentSubscriptionId=$currentSubscriptionId razorpaySubscriptionId=$razorpaySubscriptionId (empty=${razorpaySubscriptionId.isEmpty()}) -> showing failure")
            hideVerificationScreen()
            showPaymentFailedBottomSheet(getString(R.string.payment_failed_try_again))
        }
    }
    ...
    override fun onPaymentError(errorCode: Int, errorMessage: String, paymentData: PaymentData?) {
        Timber.e("onPaymentError: errorCode=$errorCode errorMessage=$errorMessage currentSubscriptionId=$currentSubscriptionId paymentData=${paymentData?.data?.toString()?.take(200)}")
        hidePaymentWebView()
        try {
            val isCancellation = errorMessage.contains("cancelled", ignoreCase = true) ||
                                errorMessage.contains("back pressed", ignoreCase = true) ||
                                errorCode == 0
            Timber.d("onPaymentError: isCancellation=$isCancellation")

            // Firebase Analytics: single event for every onPaymentError (param values truncated for FA limit)
            recordClickEvent(
                "payment_error",
                hashMapOf(
            ...
            recordClickEvent(
                if (isCancellation) "payment_cancelled" else "payment_failed",
                hashMapOf(
                    Pair("error_code", errorCode),
            ...
            // ALWAYS verify subscription status
            // Payment might have succeeded in UPI app even if:
            // - User pressed back
            // - SDK returned error
            ...
            // Try to extract payment data and use verify API
            val razorpaySubscriptionId = paymentData?.data?.optString("razorpay_subscription_id") ?: ""
            ...
            if (currentSubscriptionId != null && razorpaySubscriptionId.isNotEmpty() && razorpayPaymentId.isNotEmpty()) {
                // Have payment data - use verify API
                Timber.d("onPaymentError: have payment data -> startVerificationPolling subscriptionId=$currentSubscriptionId")
                val verifyRequest = VerifySubscriptionRequest(
                    razorpaySubscriptionId = razorpaySubscriptionId,
                    razorpayPaymentId = razorpayPaymentId,
                    razorpaySignature = signature
                ...
                // No payment data - fallback to status polling
                Timber.d("onPaymentError: no payment data -> startStatusPolling subscriptionId=$currentSubscriptionId")
                startStatusPolling(currentSubscriptionId!!)
            } else {
                Timber.e("onPaymentError: no subscriptionId -> showing failure bottom sheet")
                hideVerificationScreen()
                showPaymentFailedBottomSheet(getString(R.string.payment_failed_try_again))
            }
        } catch (e: Exception) {
            Timber.e(e, "onPaymentError: exception")
            e.printStackTrace()
            try {
                hideVerificationScreen()
                showPaymentFailedBottomSheet(getString(R.string.payment_failed_try_again))
