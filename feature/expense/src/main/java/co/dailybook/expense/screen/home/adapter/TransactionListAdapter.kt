package co.dailybook.expense.screen.home.adapter
...
import co.dailybook.expense.databinding.ItemExpenseBinding
import co.dailybook.expense.databinding.ItemNativeAdBinding
...
class TransactionListAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = getItem(position)) {
            is TransactionItem.TransactionItemView -> {
                (holder as ExpenseViewHolder).bind(item.transaction, position)
    ...
    class ExpenseViewHolder(
        ...
        fun bind(transaction: Transaction, position: Int) {
            ...
            binding.tvExpenseDate.text = dayNumber
            
            // Extract and display day of week (first 3 characters)
            val dayOfWeek = extractDayOfWeek(transaction.date)
            binding.tvExpenseDay.text = dayOfWeek
            
            binding.tvExpenseReason.text = transaction.reason

            if (transaction.type == Constants.DEBIT) {
                if (transaction.amount > 0) {
                    binding.tvCashOutAmount.text = "₹${transaction.amount}"
                }
            }
