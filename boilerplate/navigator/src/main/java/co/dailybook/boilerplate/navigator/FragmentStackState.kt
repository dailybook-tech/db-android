...
import co.dailybook.boilerplate.navigator.common.extensions.insertToBottom
import co.dailybook.boilerplate.navigator.common.extensions.moveToTop
import co.dailybook.boilerplate.navigator.data.StackItem
...
data class FragmentStackState constructor(
    ...
    fun getSelectedTabIndex() = runCatching {  tabIndexStack.peek() }.getOrDefault(0)
    ...
    fun peekItemFromSelectedTab() = runCatching {
        fragmentTagStack[getSelectedTabIndex()].takeIf { it.isNotEmpty() }?.peek() }.getOrNull()
    ...
    fun popItemsFromTabIndex(groupName: Int): List<StackItem> {
        val currentTabIndex = getSelectedTabIndex()
        val currentTabStack = fragmentTagStack.get(getSelectedTabIndex())
        ...
        for (i in 1 until currentTabStack.size) {
            val stackItem = currentTabStack[i]
            if (groupName == stackItem.tabGroup) {
                deletedStackItems.add(stackItem)
            } else {
                updatedTabStack.push(stackItem)
            }
